...
class TrametinibSingleBranchDataModule(pl.LightningDataModule):
    ...
    def _prepare_data(self):
        ...
        dmso_mask = df[dmso_col].notna()        # Has leiden value in DMSO column
        drug_mask = df[drug_col].notna()  # Has leiden value in Clonidine column
        ...
        x1_3_data = drug_data[drug_data[drug_col].astype(str) == drug_classes[2]]
        ...
        dmso_cluster_counts = dmso_data[dmso_col].value_counts()
        
        # DMSO (unchanged)
        largest_dmso_cluster = dmso_cluster_counts.index[0]
        dmso_cluster_data = dmso_data[dmso_data[dmso_col] == largest_dmso_cluster]
        ...
        if len(dmso_coords) >= target_size:
            ...
            other_dmso_data = dmso_data[dmso_data[dmso_col] != largest_dmso_cluster]
        ...
        # Class label per Trametinib cluster (0/1/2), used for classifier-free guidance conditioning
        x1_1_labels = torch.zeros(len(x1_1), dtype=torch.long)
        x1_2_labels = torch.ones(len(x1_2), dtype=torch.long)
        x1_3_labels = torch.full((len(x1_3),), 2, dtype=torch.long)
        ...
        # Stratified per-cluster split so every class appears in both train and val
        train_x1 = torch.cat([x1_1[:split_index], x1_2[:split_index], x1_3[:split_index]])
        train_x1_labels = torch.cat([x1_1_labels[:split_index], x1_2_labels[:split_index], x1_3_labels[:split_index]])
        val_x1 = torch.cat([x1_1[split_index:], x1_2[split_index:], x1_3[split_index:]])
        val_x1_labels = torch.cat([x1_1_labels[split_index:], x1_2_labels[split_index:], x1_3_labels[split_index:]])
        ...
        # x1 dataloaders carry the class label (0/1/2) in place of the old unused dummy weights
        self.train_dataloaders = {
            "x0": DataLoader(TensorDataset(train_x0, train_x0_weights), batch_size=self.batch_size, shuffle=True, drop_last=True),
            "x1": DataLoader(TensorDataset(train_x1, train_x1_labels), batch_size=self.batch_size, shuffle=True, drop_last=True),
        }

        self.val_dataloaders = {
            "x0": DataLoader(TensorDataset(val_x0, val_x0_weights), batch_size=self.batch_size, shuffle=False, drop_last=True),
            "x1": DataLoader(TensorDataset(val_x1, val_x1_labels), batch_size=self.batch_size, shuffle=True, drop_last=True),
        }
